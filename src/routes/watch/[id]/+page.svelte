				...
				<ul class="flex gap-3 flex-col">
					...
					<li>
						<div class="flex gap-3 mt-1">
							<!-- Small -->
							<img src="{character.image}" alt="User Avatar" class="inline-block w-12 h-12 rounded-full">

							<div class="flex flex-col">
							<span class="text-ascend text-sm font-gothamBook">{character.name.userPreferred}</span>
							<span class="text-slate-400 text-sm font-gothamBook">{character.voiceActors[0].name.userPreferred}</span>
						</div>
						</div>
					</li>
					{/each}
			...
			<div id="box" class="w-full flex  items-center h-40 bg-secondary p-2 rounded-2xl">
				...
				<div class="info  flex">
					...
					<img
						src="{item.image}"
						alt=""
						class="h-36 w-40 object-cover  rounded-3xl"
					/>
				</a>
					<div class="flex flex-col pl-3 justify-between it font-gotham w-full text-xs py-6 ">
						<div class="flex justify-between min-w-full ">
							<a href="/watch/{item.id}" data-sveltekit-preload-data>
					<h1 class="font-gothamMedium line-clamp-1 text-white text-sm">{item.title.english}</h1>
				</a>
					<div class="flex absolute right-2 top-2 items-center p-1  rounded-xl text-sm   h-5 bg-themeTextSecondary text-white">
						{item.rating/10}<svg
							aria-hidden="true"
				...
				<div class="flex justify-between  w-full">
					...
					<button class="p-2 absolute right-2 bottom-4  rounded-full" id="playBtn"><div
